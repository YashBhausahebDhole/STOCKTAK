...
# Function to add moving average to stock data
...
def main():
    ...
    st.title("MADE WITH LOVE by BrainWave")

    # Search bar for selecting stock by company name
    company_input = st.sidebar.text_input("Enter Company Name:")
    company_search_button = st.sidebar.button("Search Company")

    if company_search_button and company_input:
        selected_stock_symbol = get_stock_ticker(company_input)
        
        if not selected_stock_symbol:
            st.error(f"Could not find a ticker for {company_input}. Please enter a valid company name.")
        else:
            st.write(f"Found Ticker: {selected_stock_symbol}")

    else:
        selected_stock_symbol = st.sidebar.text_input("Enter Stock Ticker:")

    search_button = st.sidebar.button("Analyze Stock")

    if search_button and selected_stock_symbol:
        ...
        try:
            ...
            if stock_data.empty:
                ...
                fig.add_trace(go.Scatter(x=stock_data.index, y=stock_data['MA_20'], mode='lines', name='20-Day MA'))
                fig.update_layout(title=f'{selected_stock_symbol} Price and Moving Average',
